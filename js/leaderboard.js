 ...
 */
export function savePlayerScoreToFirebase(playerName, deviation, timeLeftSeconds, callback) {
    if (!db) {
 ...
 */
export function checkIfRecordIsTop10(playerID, callback) {
    if (!db) {
        if (callback) callback(false, null);
        return;
    }

    const limit = GAME_CONFIG.LEADERBOARD_LIMIT || 10;
    ...
    db.ref('leaderboard').once('value').then((snapshot) => {
        ...
        const isTop10 = rankIndex !== -1 && rankIndex < limit;
 ...
 */
export function listenToLeaderboardRealtime(onDataUpdated) {
    if (!db) return;
    ...
    leaderboardRef.on('value', (snapshot) => {
        ...
        const top10 = list.slice(0, limit);
