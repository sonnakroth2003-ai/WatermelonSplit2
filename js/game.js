...
const GAME_STATE = {
    ...
    soundEnabled: true,
    playerName: '',
    timeLeft: GAME_CONFIG.DEFAULT_TIME_LIMIT || 120,
    timerInterval: null,
    currentWatermelonRadius: GAME_CONFIG.WATERMELON_RADIUS || 1.5,
    isGameActive: false,
    leaderboard: [],
    planeOffset: 0.0
};
...
function triggerCut() {
    ...
    if (dragHelper) dragHelper.classList.add('opacity-0');
...
function startTimer() {
    clearInterval(GAME_STATE.timerInterval);
    GAME_STATE.timeLeft = GAME_CONFIG.DEFAULT_TIME_LIMIT || 120;
    if (canvasTimer) canvasTimer.classList.remove('hidden');

    const tick = () => {
        GAME_STATE.timeLeft--;
        const min = Math.floor(GAME_STATE.timeLeft / 60).toString().padStart(2, '0');
        const sec = (GAME_STATE.timeLeft % 60).toString().padStart(2, '0');
        if (timeLeftDisplay) timeLeftDisplay.textContent = `${min}:${sec}`;
    ...
    if (timeLeftDisplay) timeLeftDisplay.textContent = "02:00";
...
function resetGame() {
    controls.enabled = true;
    const dragHelper = document.getElementById('drag-helper');
    if (dragHelper) dragHelper.classList.remove('opacity-0');
...
window.onload = function() {
    ...
    const btnStart = document.getElementById('btn-start-cutting');
    if (btnStart) {
    ...
    const btnCut = document.getElementById('btn-cut-now');
    if (btnCut) btnCut.addEventListener('click', triggerCut);
    ...
    if (btnTimeoutRetry) {
        btnTimeoutRetry.addEventListener('click', () => {
    ...
    if (btnHelp) btnHelp.addEventListener('click', () => toggleModal('modal-help', true));

    const btnStats = document.getElementById('btn-stats');
    if (btnStats) btnStats.addEventListener('click', () => toggleModal('modal-stats', true));
    ...
    if (btnMusic) {
        btnMusic.addEventListener('click', () => {
            ...
            if (icon) icon.className = GAME_STATE.musicEnabled ? "fa-solid fa-music text-emerald-400" : "fa-solid fa-music text-slate-300";
        });
    }
    ...
    if (btnSound) {
        btnSound.addEventListener('click', () => {
            ...
            if (icon) icon.className = GAME_STATE.soundEnabled ? "fa-solid fa-volume-high text-emerald-400" : "fa-solid fa-volume-xmark text-slate-400";
        });
    }
